...
**Next: #2 (edge bundling — first cut: curvature-based routing keyed by structural
grouping, straight default retained; plan-doc now, ADR when the merged-geometry
version lands).** The standing autonomous mandate covers this whole phase (test at the
end, commit locally, never push). Below is the prior position, retained as history.
